...
graph TB
    ...
    subgraph ECS["Amazon ECS"]
        subgraph Blue["Blue Environment (Current)"]
            BlueTask[Task Set - v1]
        end
        subgraph Green["Green Environment (New)"]
            GreenTask[Task Set - v2]
        end
    ...
    Source --> Build --> Deploy
    Deploy -->|shift traffic| ALB
    ALB --> TG1 --> BlueTask
    ALB -->|cutover| TG2 --> GreenTask
    Users((Users)) --> ALB
```
...
## GitHub Source Code Repo: [HERE](https://github.com/SagarDevExpo/DevOps-Projects/tree/master/DevOps-Project-23/Swiggy_clone)
