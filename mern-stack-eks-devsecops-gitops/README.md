...
```mermaid
graph TB
    subgraph CICD["DevSecOps CI Pipeline"]
        Git[GitHub]
        Jenkins[Jenkins]
        SonarQube[SonarQube]
        Trivy[Trivy]
    ...
    Git --> Jenkins --> SonarQube --> Docker --> Trivy --> ECR
    Manifests --> ArgoCD --> EKS
    Users((Users)) --> Ingress --> React --> Node --> Mongo
```
